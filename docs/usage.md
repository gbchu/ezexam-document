...
## 注意事项
+ 由于宋体不支持加粗，故本包在设置字体时使用的是 [思源宋体](https://github.com/adobe-fonts/source-han-serif/releases)；在使用本包时，请自行下载 [思源宋体](https://github.com/adobe-fonts/source-han-serif/releases) 并安装。或者使用第三方 [粗体包](https://typst.app/universe/package/cuti)

+ 在使用 `text-figure` 方法时，如果是图片和文字混排，在给图片设置 `width` 时，不要使用 `%` 单位。特别是选项和图片排列时。如果设置百分比，图片将不显示。
